...
static void
dump_dir0 (struct tar_stat_info *st, char const *directory, struct exclude *excl)
{
  ...
  if (one_file_system_option
	...
	case exclusion_tag_none:
	  {
		...
		if (!excluded_name (name_buf, excludes))
		  dump_file (st, entry, name_buf, nexcludes);
	      }
	...
	case exclusion_tag_contents:
	  ...
	  dump_file (st, tag_file_name, name_buf, excludes);
	  free (name_buf);
	  break;
...
get_directory_entries (struct tar_stat_info *st)
{
  while (! (st->dirstream = fdopendir (st->fd)))
    if (! open_failure_recover (st))
      return 0;
  return streamsavedir (st->dirstream, SAVEDIR_SORT_NONE);
}
...
static bool
dump_dir (struct tar_stat_info *st, struct exclude* excl)
{
...
create_archive (void)
{
  ...
  if (incremental_option)
	...
	if (!excluded_name (p->name,excluded))
	  dump_file (0, p->name, p->name, excluded);
	      ...
	      while (*q)
		{
		  ...
		  if (*q == 'Y')
		      ...
		      if (buffer_size < plen + qlen)
			{
		      ...
		      dump_file (&st, q + 1, buffer, excluded);
		    }
		  q += qlen + 1;
		}
	    tar_stat_destroy (&st);
	...
	if (!excluded_name (name, excluded))
	  dump_file (0, name, name, excluded);
    }
...
static void
dump_file0 (struct tar_stat_info *st, char const *name, char const *p, struct exclude* excluded)
{
  union block *header;
  char type;
  off_t original_size;
  ...
  if (is_dir || S_ISREG (st->stat.st_mode) || S_ISCTG (st->stat.st_mode))
      ...
      if (is_dir)
	{
	  ...
	  if (check_exclusion_tags (st, &tag_file_name, NULL, NULL) == exclusion_tag_all)
	    {
	      exclusion_tag_warning (st->orig_file_name, tag_file_name,
...
dump_file (struct tar_stat_info *parent, char const *name,
	   char const *fullname, struct exclude* excluded)
{
  struct tar_stat_info st;
  tar_stat_init (&st);
  st.parent = parent;
  dump_file0 (&st, name, fullname, excluded);
